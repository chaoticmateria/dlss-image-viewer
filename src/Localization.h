...
private:
    ...
    std::wstring ReadConfiguredLanguage() const {
        const auto ini = m_base / L"DLSSImageViewer.ini";
        wchar_t value[64]{};
    ...
    static Map EnglishDefaults() {
        return {
            {L"app.title", L"DLSS Image Viewer"},
            ...
            {L"error.decode", L"Could not open or decode this video. See DLSSImageViewer.log for details."},
            {L"error.renderer", L"Could not initialize D3D12/NGX. See DLSSImageViewer.log for details."},
